...
public class TServer extends TServerSocket
{
    ...
    {
        for (SocketStreams socketStreams : clientConnections)
        {
            D obj = socketStreams.<D>read();
            if (obj != null)
            {
                return obj;
            }
    ...
    {
        return clientConnections.getKey(ip).<D>read();
    }
    ...
    {
        clientConnections.getKey(ip).write(obj);
    }
    ...
    public void _showConnectionIps()
    {
        for (SocketStreams socketStreams : clientConnections)
        {
            System.out.println(socketStreams.getConnectionIp());
        }
    }
